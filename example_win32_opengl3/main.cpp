...
void reversePolish() {
    ...
    for (int i = 0;i < len;i++) {
        ...
        if ((input[i] >= '0' && input[i] <= '9') || input[i] == '.') {
            tempin += input[i];
            std::cout << " tepmval" << tempin << "  ";
        }
        else {
            if (tempin.size() != 0) {
                output.push_back({ 0,stof(tempin),input[i] });
                std::cout << "\n add nr " << stof(tempin) << std::endl;
                tempin.clear();
            }
            if (input[i] == '(' || input[i] == '/' || input[i] == '+' || input[i] == ')') {
                ...
                if (input[i] != '(' && input[i] != ')') {
                    opord += parnr * 3;
                    std::cout << "\n opord " << opord << " " << input[i] << std::endl;
                    while (!wating.empty() && wating.top().nr >= opord) {
                        output.push_back(wating.top());
                        std::cout << "\n add symb to out" << wating.top().operation << std::endl;
                        wating.pop();
                    }
                    wating.push({ 1,(float)opord,input[i]});
                    std::cout << "\n add wating " << input[i] << std::endl;
                }
            }
        }
        //space=0;
    ...
    for (int i = 0;i < output.size();++i)
        if (!output[i].type)
            std::cout << output[i].nr;
        else
            std::cout << output[i].operation;
...
float calcSeri(float r1, float r2) {
    std::cout << std::endl << "calc serie \n";
    finalRez = r1 + r2;
    std::cout << "=" << finalRez << std::endl;
    return finalRez;
}

float calcPara(float r1, float r2) {
    std::cout << std::endl << "calc para \n";
...
void calcTotalRez() {
    ...
    finalrez = output[0].nr;
    std::cout << "\n\nfinal=" << finalrez;
    output.clear();
}
bool startR = 0;
void  ClaculateProcess()
{
    system("cls");
    std::cout << input;
    std::cout << std::endl;
    ...
    for (i;i < input.size();) {
        if (input[i] >= '0' && input[i] <= '9') {
            ...
            if (num.size() != 0) {
                std::string strval = std::to_string(rez[stoi(num) - 1]);
...
int main(int, char**)
{
    ...
    bool fullscreen2 = true;
    bool use_work_area2 = true;
    ImGuiWindowFlags flags2 = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings;
    ...
    while (!done)
    {
        ...
        ImGui::SetNextWindowPos(use_work_area2 ? viewport->WorkPos : viewport->Pos);
        ImGui::SetNextWindowSize(use_work_area2 ? viewport->WorkSize : viewport->Size);
        

        if (ImGui::Begin("Example: Fullscreen window", &fullscreen2, flags2))
        {
            ImGui::Indent();
              ImGui::Text("enter resistances:");
            ImGui::Indent();
            ...
            static float scroll_to_pos_px = 200.0f;
              ...
              float y;
                ...
                for (int item = 0; item < 60; item++)
                {
                    ...
                    char lable[4] = "R";
                    int n = log10(pos) +1;
                    for (int i = n ; i > 0; --i, pos /= 10)
                    {
                        lable[i] = (pos % 10) + '0';
                    }
                    ImGui::InputFloat(lable,&rez[item]);
                }
              ...
              ImGui::PopID();
            ImGui::Unindent();
              ImGui::Text("enter calculation:");
            ImGui::Indent();
               ImGui::InputText(" ", buf4, 100);
              input = buf4;
              ImGui::SameLine();              
              HelpMarker("rezistentele se noteaza cu r si numarul eu ex:r1,R3\n+ -> simbol pt serie\n/simbol ->paralel\nex:r1+r2/r3");
            ImGui::Unindent();
              char c[100]="";
              if (ImGui::SmallButton("calculate")) {
                  ClaculateProcess();                 
              }
              std::string textout = std::to_string(finalrez);
              for (int x = 0; x < sizeof(c) && x<textout.size(); x++) {
                  c[x] = textout[x];
              }
            ImGui::Indent();
              ImGui::Text("rezultat:");
